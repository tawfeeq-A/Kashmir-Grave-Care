              ...
              <div className="flex flex-wrap justify-center gap-4 mt-8">
                {settings?.instagram_profile_url && (
                  <a
                    ...
                    className="inline-flex items-center px-6 py-3 text-sm font-semibold text-primary bg-primary/10 hover:bg-primary/20 rounded-xl transition-all border border-primary/20"
                  >
                ...
                {settings?.facebook_profile_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center px-6 py-3 text-sm font-semibold text-blue-600 bg-blue-500/10 hover:bg-blue-500/20 rounded-xl transition-all border border-blue-500/20"
                  >
          ...
          {workMedia && workMedia.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {workMedia.map((media, idx) => (
                <Reveal key={media.id || idx} delay={idx * 0.1}>
                  <a 
                    href={media.file_url} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="group block relative overflow-hidden rounded-3xl bg-secondary/30 border border-border/60 hover:border-primary/50 transition-all hover:shadow-xl aspect-[4/5]"
                  >
                    {media.storage_path ? (
                      <img 
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-2 group-hover:translate-y-0 transition-transform">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="bg-white/20 backdrop-blur-md p-2 rounded-full text-white ml-4 shrink-0 group-hover:bg-primary transition-colors">
