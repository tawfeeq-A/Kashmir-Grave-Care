          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20 items-stretch">
            {packages.map((pkg, idx) => (
              <ScrollReveal key={pkg.name} direction="up" delay={idx * 0.15}>
                <Scroll3DTilt maxTilt={pkg.popular ? 4 : 6}>
                  <div
                    className={`relative bg-background rounded-3xl p-8 border flex flex-col justify-between transition-all duration-300 h-full cursor-pointer premium-card ${
                      pkg.popular
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {customServicesList.map((service, idx) => (
                      <ScrollReveal key={idx} direction="up" delay={idx * 0.08}>
                        <li className="flex items-start bg-background p-4 rounded-2xl border border-border/50 shadow-sm premium-card">
