...
class ShadcnInstaller:
    ...
    def get_installed_components(self) -> List[str]:
        ...
        try:
            ...
            return [f.stem for f in ui_dir.iterdir() if f.is_file() and f.suffix in ('.tsx', '.jsx')]
        except (json.JSONDecodeError, KeyError, OSError):
            return []

    def _get_shadcn_version(self) -> str:
        """Read shadcn version from project package.json; fall back to 'latest'."""
            ...
            try:
                ...
                if isinstance(pkg, dict):
                    for section in ("dependencies", "devDependencies"):
                        sec_dict = pkg.get(section)
                        if isinstance(sec_dict, dict):
                            version = sec_dict.get("shadcn")
                            if version and isinstance(version, str):
                                return version.lstrip("^~>=<").split()[0]
            except (json.JSONDecodeError, KeyError, TypeError):
                pass
        return "latest"  # fallback to track latest stable release
