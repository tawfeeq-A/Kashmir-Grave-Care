...
def generate_html(brand_name, industry, images_dir, output_path=None, style=None):
    ...
    if not images_dir.exists():
        sys.stderr.write(f"Error: Directory not found: {images_dir}\n")
        sys.exit(1)

    # Get all PNG images
    images = sorted(images_dir.glob("*.png"))
    if not images:
        sys.stderr.write(f"Error: No PNG images found in {images_dir}\n")
        sys.exit(1)
...
<head>
    ...
    <title>{_e(brand_name)} - Corporate Identity Program</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
    ...
    <section class="hero">
        ...
        <div class="meta">
            <div class="meta-item">
                ...
                <div class="meta-value">{_e(industry_info.get("Industry", industry.title()))}</div>
            </div>
            <div class="meta-item">
                <div class="meta-label">Style</div>
                <div class="meta-value">{_e(style_info.get("Style Name", "Corporate"))}</div>
            </div>
            <div class="meta-item">
                <div class="meta-label">Mood</div>
                <div class="meta-value">{_e(style_info.get("Mood", "Professional"))}</div>
            </div>
            <div class="meta-item">
    ...
    for i, image_path in enumerate(images):
        ...
        <div class="deliverable">
            <div class="deliverable-image">
                <img src="{img_src}" alt="{_e(info['title'])}" loading="lazy">
            </div>
            <div class="deliverable-content">
                <h3 class="deliverable-title">{_e(info['title'])}</h3>
                <p class="deliverable-concept">{_e(info['concept'])}</p>
                <p class="deliverable-purpose">{_e(info['purpose'])}</p>
                <span class="deliverable-specs">{_e(info['specs'])}</span>
            </div>
        </div>
''')
    ...
    <footer class="footer">
        <p><strong>{_e(brand_name)}</strong> Corporate Identity Program</p>
...
''')
    ...
    try:
        ...
        sys.stderr.write(f"Error writing to output file {output_path}: {e}\n")
        sys.exit(1)
