...
def get_layout_for_goal(goal, previous_emotion=None):
    ...
    if result.get("break_pattern") == "true" and previous_emotion:
        current_emotion = result.get("emotion_beat")
        contrasting_emotions = {
            "frustration": ["hope", "relief"],
            "hope": ["frustration", "fear"],
            "fear": ["hope", "relief"],
            "relief": ["frustration", "fear"],
            "urgency": ["calm", "relief"],
        }
        is_contrast = False
        if current_emotion and previous_emotion:
            if previous_emotion in contrasting_emotions and current_emotion in contrasting_emotions[previous_emotion]:
                is_contrast = True
            elif current_emotion in contrasting_emotions and previous_emotion in contrasting_emotions[current_emotion]:
                is_contrast = True
        ...
        if is_contrast:
            ...
            result["break_pattern"] = "false"
            if "_pattern_break" in result:
                del result["_pattern_break"]
...
def should_use_full_bleed(slide_index, total_slides, emotion):
    ...
    if total_slides <= 6:
        # Space them out to avoid adjacent candidates in short decks
        strategic_positions = [1, 3, total_slides] if total_slides >= 5 else [1, total_slides]
    else:
        pos1 = 1
        pos2 = third
        pos3 = third * 2
        pos4 = total_slides
        # Adjust to avoid adjacency
        if pos2 <= pos1 + 1:
            pos2 = pos1 + 2
        if pos3 <= pos2 + 1:
            pos3 = pos2 + 2
        if pos4 <= pos3 + 1:
            pos4 = pos3 + 2
        strategic_positions = [pos1, pos2, pos3, pos4]

    return slide_index in strategic_positions


def calculate_pattern_break(slide_index, total_slides, current_emotion=None, previous_emotion=None):
    """
    ...
    if previous_emotion and current_emotion:
        if previous_emotion in contrasting_emotions and current_emotion in contrasting_emotions[previous_emotion]:
            return True
        if current_emotion in contrasting_emotions and previous_emotion in contrasting_emotions[current_emotion]:
            return True

    return False


def detect_slide_goal(query):
    """Detect the slide goal from the query to map to slide layout/typography logics"""
    query_lower = query.lower()
    goal_keywords = {
        "problem": ["problem", "pain", "need", "challenge"],
        "solution": ["solution", "how it works", "value prop"],
        "cta": ["cta", "call to action", "closing", "next steps", "contact"],
        "hook": ["hook", "title", "opening", "intro"],
        "traction": ["traction", "metric", "growth", "revenue", "numbers", "kpi"],
        "pricing": ["pricing", "price", "cost", "plan", "business model"],
        "team": ["team", "founders", "about us", "people"],
        "comparison": ["comparison", "versus", "vs", "competitor", "alternative"],
        "timeline": ["timeline", "roadmap", "milestones", "schedule", "plan"],
        "features": ["features", "benefits", "product", "technology"],
        "testimonial": ["testimonial", "quote", "customer", "reviews", "case study", "social proof"]
    }
...
def search_with_context(query, slide_position=1, total_slides=9, previous_emotion=None):
    ...
    # Get color treatment — use "emotion_beat" (CSV-backed key set by get_layout_for_goal)
    emotion = layout.get("emotion_beat", "clarity") if layout else "clarity"
    ...
    context["should_break_pattern"] = calculate_pattern_break(
        slide_position, total_slides, emotion, previous_emotion
    )
