...
def generate_chart_slide(data):
    ...
    for b in bars:
        ...
        display_val = b.get('display', str(val) + '%')
        bars_html.append(f'''
        <div class="bar" style="height: {parsed_val}%;">
            <span class="bar-value">{_e(display_val)}</span>
...
def generate_deck(slides_data, title="Pitch Deck", output_path=None):
    ...
    import os
    if output_path:
        tokens_rel_path = os.path.relpath(TOKENS_CSS, os.path.dirname(output_path)).replace("\\", "/")
    else:
...
def main():
    ...
    if args.demo:
        ...
        html = generate_deck(demo_slides, "ClaudeKit Marketing - Pitch Deck", output_path)
        ...
        output_path = output_path.resolve()
        html = generate_deck(data.get('slides', []), data.get('title', 'Presentation'), output_path)
        output_path.write_text(html, encoding='utf-8')
