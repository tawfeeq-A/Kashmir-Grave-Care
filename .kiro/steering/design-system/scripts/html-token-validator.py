...
FORBIDDEN_PATTERNS = [
    ...
    (r'hsl\([^)]+\)', 'hsl color'),
    (r"font-family:\s*['\"][^'\"]+['\"]", 'hardcoded font'),
]
...
_DYNAMIC_RGBA_PATTERNS = None

def is_allowed_rgba(match_text: str) -> bool:
    """Check if rgba pattern uses brand colors (allowed for transparency)."""
    global _DYNAMIC_RGBA_PATTERNS
    if _DYNAMIC_RGBA_PATTERNS is None:
        def hex_to_rgba_pattern(hex_val: str) -> Optional[str]:
        ...
        for val in css_vars.values():
            for h in re.findall(r'#[0-9A-Fa-f]{3,6}\b', val):
                p = hex_to_rgba_pattern(h)
                if p and p not in patterns:
                    patterns.append(p)
                    
        # 2. From design-tokens-starter.json and design-tokens.json
        starter_json_path = PROJECT_ROOT / '.kiro' / 'steering' / 'design-system' / 'templates' / 'design-tokens-starter.json'
        tokens_json_path = PROJECT_ROOT / 'assets' / 'design-tokens.json'
        
        def extract_colors_from_dict(d):
            found = []
            if isinstance(d, dict):
                for k, v in d.items():
                    if isinstance(v, str) and (v.startswith('#') or 'rgb' in v):
                        found.append(v)
                    else:
                        found.extend(extract_colors_from_dict(v))
            elif isinstance(d, list):
                for item in d:
                    found.extend(extract_colors_from_dict(item))
            return found

        for path in [starter_json_path, tokens_json_path]:
            if path.exists():
                try:
                    data = json.loads(path.read_text())
                    for val in extract_colors_from_dict(data):
                        for h in re.findall(r'#[0-9A-Fa-f]{3,6}\b', val):
                            p = hex_to_rgba_pattern(h)
                            if p and p not in patterns:
                                patterns.append(p)
                    ...
                    pass
        _DYNAMIC_RGBA_PATTERNS = patterns

    return any(re.match(pattern, match_text) for pattern in _DYNAMIC_RGBA_PATTERNS)
