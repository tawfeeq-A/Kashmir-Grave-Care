...
- Form controls must use the native `disabled` attribute (which natively handles focus and interaction prevention)
- Custom controls must combine `aria-disabled="true"`, JavaScript event guarding (ignoring clicks/keys), and focus prevention (`tabindex="-1"`)
