...
def on_color(bg):
    ...
    contrast_white = (l_white + 0.05) / (l_bg + 0.05)
    
    if l_bg >= l_dark:
        contrast_dark = (l_bg + 0.05) / (l_dark + 0.05)
    else:
        contrast_dark = (l_dark + 0.05) / (l_bg + 0.05)
        
    return "#FFFFFF" if contrast_white >= contrast_dark else "#0F172A"
...
def rebuild_colors():
    ...
    try:
        with open(tmp_src, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=headers)
            writer.writeheader()
            writer.writerows(final_rows)
            
        if os.path.exists(src):
            os.replace(src, backup_src)
            backup_created = True
            
        os.replace(tmp_src, src)
        
        if backup_created and os.path.exists(backup_src):
            os.remove(backup_src)
    except Exception as e:
        if backup_created and os.path.exists(backup_src):
            try:
                os.replace(backup_src, src)
            except Exception:
                pass
    ...
    print(f"\n  [SUCCESS] colors.csv: {len(final_rows)} rows ({product_count} products)")
...
def rebuild_ui_reasoning():
    ...
    print(f"\n  [SUCCESS] ui-reasoning.csv: {len(final_rows)} rows")
