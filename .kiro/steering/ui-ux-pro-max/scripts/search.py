...
if __name__ == "__main__":
    ...
    if args.design_system and args.json:
        sys.stderr.write("Error: --json cannot be combined with --design-system\n")
        sys.exit(1)
    ...
    if args.design_system:
        ...
        if args.persist:
            result, persist_info = res
        else:
            result = res
        print(result)
        
        # Print persistence confirmation
        if args.persist and persist_info:
            import os
            design_system_dir = persist_info.get("design_system_dir")
            created_files = persist_info.get("created_files", [])
            print("\n" + "=" * 60)
            print(f"✅ Design system persisted to {design_system_dir}/")
            for filepath in created_files:
                fname = os.path.basename(filepath)
                if fname == "MASTER.md":
                    print(f"   📄 {filepath} (Global Source of Truth)")
                else:
                    print(f"   📄 {filepath} (Page Overrides)")
            print("")
            print(f"📖 Usage: When building a page, check the page overrides file first.")
