...
def section_header(name: str, width: int) -> str:
    ...
    prefix = "├─── "
    suffix = "─┤"
    fixed_len = len(prefix) + len(suffix) # 7
    if len(name) + fixed_len > width:
        # Truncate name
        available = width - fixed_len - 3 # for "..."
        if available > 0:
            name = name[:available] + "..."
        else:
            return "─" * width
    ...
    label = f"─── {name} "
    fill_len = width - len(label) - 2 # 1 for ├, 1 for ┤
    fill = "─" * max(0, fill_len)
    return f"├{label}{fill}┤"
...
def generate_design_system(query: str, project_name: str = None, output_format: str = "ascii",
    ...
    if persist:
        persist_info = persist_design_system(design_system, page, output_dir, query)

    if output_format == "markdown":
        output_str = format_markdown(design_system)
    else:
        output_str = format_ascii_box(design_system)

    if persist:
        return output_str, persist_info
    return output_str


def sanitize_filename_part(text: str) -> str:
    """Sanitize a string to be safe for filenames by removing any character not alphanumeric, hyphen, or underscore"""
    import re
...
def persist_design_system(design_system: dict, page: str = None, output_dir: str = None, page_query: str = None) -> dict:
    ...
    project_slug = sanitize_filename_part(project_name)
    if not project_slug:
    ...
    if page:
        page_slug = sanitize_filename_part(page)
        if not page_slug:
            page_slug = "override"
