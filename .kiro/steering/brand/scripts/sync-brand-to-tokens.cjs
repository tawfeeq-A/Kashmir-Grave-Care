...
const GENERATE_TOKENS_SCRIPT = '.kiro/steering/design-system/scripts/generate-tokens.cjs';
  ...
  const assignFromSection = (heading, target) => {
    ...
    for (const m of section[0].matchAll(rowRe)) {
      ...
      if (label.includes('dark')) target.dark = hex;
        ...
        const rowName = m[1].trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        if (!target.base) {
          target.base = hex;
          target.name = rowName;
        } else {
          // Always derive name from the descriptive label
          target.name = rowName;
          if (target.base !== hex) {
            console.warn(`⚠️  Table hex ${hex} differs from Quick Reference ${target.base} for ${rowName} — keeping Quick Reference value.`);
          }
        }
      }
    }
  ...
  if (!colors.accent.base) {
    for (const m of content.matchAll(rowRe)) {
      if (m[1].trim().toLowerCase().includes('accent')) {
        ...
        colors.accent.name = m[1].trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        break;
      }
    }
  }
...
function updateDesignTokens(tokens, colors, brandName) {
  ...
  // Load the prior managed-name set, default to ['coral', 'purple', 'mint']
  const priorManaged = tokens.metadata?.managedColors || ['coral', 'purple', 'mint'];
  priorManaged.forEach(name => {
    delete primitiveColors[name];
  });
  ...
  for (const role of ['primary', 'secondary', 'accent']) {
    ...
    newManaged.push(c.name);
  }

  tokens.metadata = tokens.metadata || {};
  tokens.metadata.managedColors = newManaged;
...
function main() {
  ...
  const h1Match = guidelinesContent.match(/^#\s+(.+)$/m);
  const brandName = h1Match ? h1Match[1].trim() : "Default Brand";

  // Update tokens
  tokens = updateDesignTokens(tokens, colors, brandName);
