...
4. **Archive don't delete** - Archive superseded or duplicate assets in `assets/archived/` with a `YYYYMMDD` date prefix, and update their manifest status to `"archived"` rather than removing them from the workspace. The `assets/archived/` directory is owned by the brand team and organized by asset type subdirectories.
...
### By Campaign
```bash
# Find all assets for specific campaign by querying manifest contents
jq '.assets[] | select(.campaign == "claude-launch")' .assets/manifest.json
```
...
4. Tag appropriately
5. Archive duplicates/outdated versions instead of removing them (move to archived/ with YYYYMMDD date prefix and update manifest status to "archived")
